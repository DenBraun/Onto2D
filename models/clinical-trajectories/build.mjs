...
import { compileClinicalTrajectoriesModelPack } from "./compiler.mjs";
...
export async function buildClinicalTrajectoriesRelease() {
  return compileClinicalTrajectoriesModelPack(await buildClinicalTrajectoriesCase());
}

export async function writeClinicalTrajectoriesRelease(pack) {
...
export async function verifyClinicalTrajectoriesRelease(expected) {
...
export async function run({ verify = false } = {}) {
  const pack = await buildClinicalTrajectoriesRelease();
  if (!verify) await writeClinicalTrajectoriesRelease(pack);
  await verifyClinicalTrajectoriesRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Clinical Trajectories Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
