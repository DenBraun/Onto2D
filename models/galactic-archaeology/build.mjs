...
export async function buildGalacticArchaeologyRelease() { return compileGalacticArchaeologyModelPack(await buildGalacticArchaeologyCase()); }

export async function writeGalacticArchaeologyRelease(pack) {
...
export async function verifyGalacticArchaeologyRelease(expected) {
...
export async function run({ verify = false } = {}) {
  const pack = await buildGalacticArchaeologyRelease();
  if (!verify) await writeGalacticArchaeologyRelease(pack);
  await verifyGalacticArchaeologyRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Galactic Archaeology Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
