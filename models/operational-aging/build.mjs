...
export async function buildOperationalAgingRelease() { return compileOperationalAgingModelPack(await buildOperationalAgingCase()); }
export async function writeOperationalAgingRelease(pack) {
  ...
  for (const [relative, value] of Object.entries(pack.files)) { const target = path.join(releaseRoot, relative); await mkdir(path.dirname(target), { recursive: true }); await writeFile(target, serialize(value)); }
  await writeFile(path.join(releaseRoot, "bundle.json"), serialize(pack));
  return releaseRoot;
}
export async function verifyOperationalAgingRelease(expected) {
...
export async function run({ verify = false } = {}) {
  ...
  if (!verify) await writeOperationalAgingRelease(pack);
  await verifyOperationalAgingRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Operational Aging Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
