...
export async function buildMaterialProcessHistoryRelease() { return compileMaterialProcessHistoryModelPack(await buildMaterialProcessHistoryCase()); }

export async function writeMaterialProcessHistoryRelease(pack) {
...
export async function verifyMaterialProcessHistoryRelease(expected) {
...
export async function run({ verify = false } = {}) {
  const pack = await buildMaterialProcessHistoryRelease();
  if (!verify) await writeMaterialProcessHistoryRelease(pack);
  await verifyMaterialProcessHistoryRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Material Process History Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
