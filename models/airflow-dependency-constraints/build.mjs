...
export async function buildAirflowDependencyConstraintsRelease() {
  return compileAirflowDependencyConstraintsModelPack(await buildAirflowDependencyConstraintsCase());
}

export async function writeAirflowDependencyConstraintsRelease(pack) {
...
export async function verifyAirflowDependencyConstraintsRelease(expected) {
  const releaseRoot = path.join(MODEL_ROOT, "releases", expected.manifest.model.version);
...
export async function run({ verify = false } = {}) {
  const pack = await buildAirflowDependencyConstraintsRelease();
  if (!verify) await writeAirflowDependencyConstraintsRelease(pack);
  await verifyAirflowDependencyConstraintsRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Airflow Dependency Constraints Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
