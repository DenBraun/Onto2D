...
export async function buildInTotoProvenanceRelease() {
  return compileInTotoProvenanceModelPack(await buildInTotoAdmissibilityCase());
}

export async function writeInTotoProvenanceRelease(pack) {
...
export async function verifyInTotoProvenanceRelease(expectedPack) {
  const releaseRoot = path.join(MODEL_ROOT, "releases", expectedPack.manifest.model.version);
  assert.deepEqual(await collectFiles(releaseRoot), ["bundle.json", "manifest.json", ...Object.keys(expectedPack.files)].sort(), "The in-toto release contains missing or unexpected files.");
...
export async function run({ verify = false } = {}) {
  ...
  if (!verify) await writeInTotoProvenanceRelease(pack);
  await verifyInTotoProvenanceRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} in-toto Provenance Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
