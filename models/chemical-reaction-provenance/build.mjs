...
export async function buildChemicalReactionProvenanceRelease() {
  return compileChemicalReactionProvenanceModelPack(await buildChemicalSynthesisHistoryCase());
}

export async function writeChemicalReactionProvenanceRelease(pack) {
...
export async function verifyChemicalReactionProvenanceRelease(expectedPack) {
  const releaseRoot = path.join(MODEL_ROOT, "releases", expectedPack.manifest.model.version);
  assert.deepEqual(await collectFiles(releaseRoot), ["bundle.json", "manifest.json", ...Object.keys(expectedPack.files)].sort(), "The chemical reaction release contains missing or unexpected files.");
...
export async function run({ verify = false } = {}) {
  ...
  if (!verify) await writeChemicalReactionProvenanceRelease(pack);
  await verifyChemicalReactionProvenanceRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Chemical Reaction Provenance Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
