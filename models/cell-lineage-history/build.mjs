...
import { compileCellLineageHistoryModelPack } from "./compiler.mjs";
...
export async function buildCellLineageHistoryRelease() { return compileCellLineageHistoryModelPack(await buildCellLineageIdentityCase()); }

export async function writeCellLineageHistoryRelease(pack) {
...
export async function verifyCellLineageHistoryRelease(expected) {
...
export async function run({ verify = false } = {}) {
  const pack = await buildCellLineageHistoryRelease();
  if (!verify) await writeCellLineageHistoryRelease(pack);
  await verifyCellLineageHistoryRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Cell Lineage Identity Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
