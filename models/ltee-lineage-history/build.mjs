...
export async function buildLteeLineageHistoryRelease() { return compileLteeLineageHistoryModelPack(await buildLteeEvolutionaryContingencyCase()); }

export async function writeLteeLineageHistoryRelease(pack) {
...
export async function verifyLteeLineageHistoryRelease(expected) {
...
export async function run({ verify = false } = {}) {
  const pack = await buildLteeLineageHistoryRelease();
  if (!verify) await writeLteeLineageHistoryRelease(pack);
  await verifyLteeLineageHistoryRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} LTEE Lineage History Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
