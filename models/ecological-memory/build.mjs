...
import { compileEcologicalMemoryModelPack } from "./compiler.mjs";
...
export async function buildEcologicalMemoryRelease() { return compileEcologicalMemoryModelPack(await buildEcologicalMemoryCase()); }

export async function writeEcologicalMemoryRelease(pack) {
...
export async function verifyEcologicalMemoryRelease(expected) {
...
export async function run({ verify = false } = {}) {
  const pack = await buildEcologicalMemoryRelease();
  if (!verify) await writeEcologicalMemoryRelease(pack);
  await verifyEcologicalMemoryRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Ecological Memory Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
