...
export async function buildOciLayerProvenanceRelease() {
  return compileOciLayerProvenanceModelPack(await buildOciLayerHistoryCase());
}

export async function writeOciLayerProvenanceRelease(pack) {
...
export async function verifyOciLayerProvenanceRelease(expectedPack) {
  const releaseRoot = path.join(MODEL_ROOT, "releases", expectedPack.manifest.model.version);
  const expectedFiles = ["bundle.json", "manifest.json", ...Object.keys(expectedPack.files)].sort();
  assert.deepEqual(await collectFiles(releaseRoot), expectedFiles, "The OCI release contains missing or unexpected files.");
...
export async function run({ verify = false } = {}) {
  ...
  if (!verify) await writeOciLayerProvenanceRelease(pack);
  await verifyOciLayerProvenanceRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} OCI Layer History Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
