...
export async function buildManuscriptTransmissionRelease() { return compileManuscriptTransmissionModelPack(await buildManuscriptStemmaticsCase()); }
export async function writeManuscriptTransmissionRelease(pack) {
...
export async function verifyManuscriptTransmissionRelease(expected) {
...
export async function run({ verify = false } = {}) {
  ...
  if (!verify) await writeManuscriptTransmissionRelease(pack);
  await verifyManuscriptTransmissionRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Manuscript Transmission Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
