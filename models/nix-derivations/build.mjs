...
export async function buildNixDerivationsRelease() {
  return compileNixDerivationsModelPack(await buildNixDerivationCase());
}

export async function writeNixDerivationsRelease(pack) {
...
export async function verifyNixDerivationsRelease(expectedPack) {
  ...
  assert.deepEqual(await collectFiles(releaseRoot), expectedFiles, "The Nix release contains missing or unexpected files.");
...
export async function run({ verify = false } = {}) {
  ...
  if (!verify) await writeNixDerivationsRelease(pack);
  await verifyNixDerivationsRelease(pack);
  console.log(`${verify ? "Verified" : "Built"} Nix Derivation Identity Model Pack ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} nodes, ${pack.manifest.statistics.edgeCount} edges, ${pack.manifest.rootHash}`);
  return pack;
}
