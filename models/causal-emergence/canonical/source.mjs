...
import { validateDictionaryReview, buildQuantitativeCensus } from "./dictionaries.mjs";
...
const dictionaryPolicyBytes = await readFile(path.join(ROOT, "references/canonical/dictionary-policy.json"));
...
export function validateCanonicalSource(data, legacyLevels) {
  ...
  validateDictionaryReview(data, legacyDictionaries, dictionaryPolicyBytes);
  return data;
}

export async function loadCanonicalSource() {
  const [graph, migration, edgeReviews, relationPolicy, pilot, pilotPolicy, routing, routingPolicy, dictionaryReview, dictionaryPolicy, ...legacyLevels] = await Promise.all([
    ...
    json("references/canonical/dictionary-review.json"), json("references/canonical/dictionary-policy.json"),
    ...Array.from({ length: 8 }, (_, i) => json(`references/level-${i}.json`))
  ]);
  const data = validateCanonicalSource({ graph, migration, edgeReviews, relationPolicy, pilot, pilotPolicy, routing, routingPolicy, dictionaryReview, dictionaryPolicy }, legacyLevels);
...
export async function loadQuantitativeCensus() {
  const legacyLevels = await Promise.all(Array.from({ length: 8 }, (_, i) => json(`references/level-${i}.json`)));
  return buildQuantitativeCensus(legacyLevels, legacyDictionaries);
}
