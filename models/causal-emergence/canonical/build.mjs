...
import { ROOT, loadCanonicalSource, verifyCanonicalEvidence, loadQuantitativeCensus } from "./source.mjs";
import { compileVocabulary } from "./dictionaries.mjs";

const serialize = (value) => `${JSON.stringify(value, null, 2)}\n`;

export function compileCanonicalGraph({ graph, migration, edgeReviews, relationPolicy, pilot, pilotPolicy, routing, routingPolicy, dictionaryReview, dictionaryPolicy }, evidence, sourceFiles, quantitativeCensus) {
  assert.ok(quantitativeCensus?.records.length === migration.edges.length, "Missing quantitative accounting");
  ...
  return buildModelPack({
    ...
    source: { id: "canonical-research-source-v5", files: sourceFiles, auditHash: hashCanonical("onto2d:canonical-reconstruction-evidence:v1", evidence) },
    nodes, edges,
    dictionaries: {
      ...
      dictionaryReview, dictionaryPolicy, vocabulary: compileVocabulary(dictionaryReview, graph.sources), quantitativeCensus,
      presentation: {
        ...
        boundary: {
          ...
          note: `${nodes.length} records. ${edgeReviews.records.length + pilot.edgeReviews.length} historical assertions have internal decisions; ${migration.edges.filter((e) => e.status === "pending-scientific-review").length} remain pending. Separately, ${dictionaryReview.records.length} dictionary entries have semantic decisions; all ${quantitativeCensus.records.length} original quantitative assertions remain withheld. Levels are display groups.`
        }
      }
    }
  });
...
export async function buildCanonicalRelease() {
  ...
  return compileCanonicalGraph(data, evidence, files, await loadQuantitativeCensus());
}
