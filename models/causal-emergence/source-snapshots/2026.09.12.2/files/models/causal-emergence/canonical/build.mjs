...
export function compileCanonicalGraph({ graph, migration, edgeReviews, relationPolicy, pilot, pilotPolicy }, evidence, sourceFiles) {
  const claims = new Map(graph.claims.map((c) => [c.id, c]));
  const sources = new Map(graph.sources.map((s) => [s.id, s]));
  const studies = new Map(pilot.studies.map((s) => [s.id, s]));
  function rationale(ids) {
    return ids.map((id) => {
      const claim = claims.get(id);
      return { ...claim, contexts: (claim.contextIds ?? []).map((id) => studies.get(id)), citations: claim.citations.map((c) => ({ ...c, source: sources.get(c.sourceId) })) };
    });
  }
  ...
  return buildModelPack({
    ...
    source: { id: "canonical-research-source-v3", files: sourceFiles, auditHash: hashCanonical("onto2d:canonical-reconstruction-evidence:v1", evidence) },
    nodes, edges,
    dictionaries: {
      sources: graph.sources, claims: graph.claims, constructionRules: graph.rules,
      scope: graph.scope, migration, evidence, edgeReviews, relationPolicy, pilot, pilotPolicy,
      presentation: {
        ...
        boundary: {
          title: "Canonical reconstruction · Level 0 and retinal pilot",
          summary: "Conditional definitions and scoped published observations. The retinal fragments use different preparations; no validated whole-graph model or physical CRT instance is claimed.",
