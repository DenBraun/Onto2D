...
export async function loadCanonicalSource() {
  const [graph, migration, edgeReviews, relationPolicy, pilot, pilotPolicy, ...legacyLevels] = await Promise.all([
  ...
  const data = validateCanonicalSource({ graph, migration, edgeReviews, relationPolicy, pilot, pilotPolicy }, legacyLevels);
