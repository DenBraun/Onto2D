...
export function validateCanonicalSource(data, legacyLevels) {
  ...
  const { graph, migration, edgeReviews, relationPolicy } = data;
  ...
  const reviews = index(edgeReviews.records, "relation review");
  ...
  for (const relation of relations.values()) {
    ...
    else assert.ok(relation.claimIds.some((id) => claims.get(id).kind === "author-definition" && claims.get(id).status === "definition"), `New definition lacks a definition claim ${relation.id}`);
  }
  const expectedNodes = [];
  const expectedEdges = [];
...
export async function loadCanonicalSource() {
  const [graph, migration, edgeReviews, relationPolicy, ...legacyLevels] = await Promise.all([
  ...
  const data = validateCanonicalSource({ graph, migration, edgeReviews, relationPolicy }, legacyLevels);
