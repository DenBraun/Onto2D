...
export function compileCanonicalGraph({ graph, migration, edgeReviews, relationPolicy }, evidence, sourceFiles) {
  ...
  return buildModelPack({
    ...
    source: { id: "canonical-research-source-v2", files: sourceFiles, auditHash: hashCanonical("onto2d:canonical-reconstruction-evidence:v1", evidence) },
    nodes, edges,
    dictionaries: {
      sources: graph.sources, claims: graph.claims, constructionRules: graph.rules,
      scope: graph.scope, migration, evidence, edgeReviews, relationPolicy,
      presentation: {
        ...
        boundary: {
          ...
          note: `${nodes.length} specification records. ${edgeReviews.records.length} Level-0 assertions have internal review decisions; ${migration.edges.filter((e) => e.status === "pending-scientific-review").length} other assertions remain pending. Earlier releases are preserved.`
        }
      }
    }
  });
...
export async function run({ verify = false, version = null } = {}) {
  ...
  console.log(`${verify ? "Verified" : "Built"} canonical reconstruction ${pack.manifest.model.version}: ${pack.manifest.statistics.nodeCount} records, ${pack.manifest.statistics.edgeCount} descriptive connections; ${pack.manifest.rootHash}`);
  return pack;
}
