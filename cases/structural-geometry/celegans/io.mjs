...
export async function readJson(path, { hash = null, maximum = 128000000 } = {}) {
  ...
  if (!info.isFile() || info.isSymbolicLink() || info.size > maximum) throw new Error("Expected a bounded regular D5 input.");
  const bytes = await readFile(url);
  if (bytes.length > maximum || (hash !== null && sha256(bytes) !== hash)) throw new Error(`D5 source binding differs: ${path}`);
  return JSON.parse(bytes);
}
export async function realDirectories(paths) {
  for (const path of paths) {
    const info = await lstat(fileURLToPath(new URL(path, HERE)).replace(/\/$/, ""));
    if (!info.isDirectory() || info.isSymbolicLink()) throw new Error("D5 input/output directories must be real.");
  }
}
export async function writeJson(path, value, { pretty = false } = {}) {
  if (!["cache/prepared.json", "cache/details.json", "results.json", "costs.json"].includes(path)) throw new Error("Unknown D5 artifact destination.");
  ...
  if (existing && (!existing.isFile() || existing.isSymbolicLink())) throw new Error("D5 output must be a regular file.");
...
export function python(script, args, input, { timeout = 600000, maximum = 128000000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => stop(new Error(`D5 ${script} exceeded its execution limit.`)), timeout);
    child.on("error", error => { clearTimeout(timer); reject(error); });
    child.stdin.on("error", error => { if (error.code !== "EPIPE") stop(error); });
    child.stdout.on("data", bytes => {
      size += bytes.length;
      if (size > maximum) stop(new Error("D5 Python output exceeds the complete-artifact bound."));
      else buffers.push(bytes);
    });
    child.stderr.on("data", bytes => { stderr = (stderr + bytes).slice(-10000); });
    child.on("close", code => {
      clearTimeout(timer);
      if (failure || code !== 0) reject(failure ?? new Error(`D5 ${script} failed: ${stderr}`));
