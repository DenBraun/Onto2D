...
export const input = Object.freeze({ regimeId: "topology-only-v1" });
export function packFor(graph) {
  const labels = graph.labels ?? Array.from({ length: graph.nodes }, (_, i) => `n${i}`);
  return buildModelPack({ model: { id: `topology-${graph.id}`, name: "Declared directed topology control", version: "1" },
...
export async function fixtures() {
  const { controls, causalScopes } = await readJson("controls.json");
  ...
  return [...controls.map((graph) => ({ id: graph.id, pack: packFor(graph), input })),
    ...causalScopes.map(({ id, nodeIds }) => ({ id, pack: causal, input: { ...input, scope: { kind: "induced", nodeIds } } }))];
}
