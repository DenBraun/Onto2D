...
export async function readJson(path, { hash = null, maximum = 128000000 } = {}) {
  ...
  if (!info.isFile() || info.isSymbolicLink() || info.size > maximum) throw new Error("Expected a bounded regular D6.4 input.");
  const bytes = await readFile(url);
  if (bytes.length > maximum || (hash !== null && sha256(bytes) !== hash)) throw new Error(`D6.4 source binding differs: ${path}`);
  return JSON.parse(bytes);
}
export async function writeJson(path, value, { pretty = false } = {}) {
  if (!["results.json", "costs.json", "controls.json"].includes(path) && !/^cache\/[a-z-]+\.json$/.test(path)) throw new Error("Unknown D6.4 artifact destination.");
  ...
  for (const name of [".", ...(path.startsWith("cache/") ? ["cache"] : [])]) {
    ...
    if (!info.isDirectory() || info.isSymbolicLink()) throw new Error("D6.4 output directories must be real.");
  }
  ...
  if (existing && (!existing.isFile() || existing.isSymbolicLink())) throw new Error("D6.4 output must be a regular file.");
...
export function python(mode, input, { networkx = false, timeout = 600000 } = {}) {
  return new Promise((resolve, reject) => {
    const executable = networkx ? process.env.ONTO2D_METRIC_REFERENCE_PYTHON ?? "python3" : "python3";
    ...
    const timer = setTimeout(() => stop(new Error("D6.4 independent reference exceeded its execution limit.")), timeout);
    child.on("error", error => { clearTimeout(timer); reject(error); });
    child.stdin.on("error", error => { if (error.code !== "EPIPE") stop(error); });
    child.stdout.on("data", bytes => { size += bytes.length; if (size > 4000000) stop(new Error("D6.4 reference output exceeds bound.")); else buffers.push(bytes); });
    child.stderr.on("data", bytes => { stderr = (stderr + bytes).slice(-10000); });
    child.on("close", code => {
      clearTimeout(timer);
      if (failure || code !== 0) reject(failure ?? new Error(`D6.4 independent reference failed: ${stderr}`));
